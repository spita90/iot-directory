...
req.open("POST", link+"?action=get_param_values", true);
...
var connLink = apiLink+"deviceDiscoveryApi.php?action=getCBServiceTree&contextbroker="+ORION_CB;
req.open("POST", connLink, true);
...
/*      //LATLONG DUMMY DATA
                            se.push(storeDevice(USER, topic, MODEL, KIND, devAttr["devicetype"],
                                ...
                                devAttr["longitude"], devAttr["mac"], validity, verify.message, "no", ORGANIZATION, EDGE_GATEWAY_TYPE, EDGE_GATEWAY_URI, devAttr["k1"], devAttr["k2"], service, servicePath));
                           */
                            se.push(storeDevice(USER, topic, MODEL, KIND, devAttr["devicetype"],
                                ORION_PROTOCOL, FREQUENCY, devAttr["format"], ORION_CB, 43,
                                11, devAttr["mac"], validity, verify.message, "no", ORGANIZATION, EDGE_GATEWAY_TYPE, EDGE_GATEWAY_URI, devAttr["k1"], devAttr["k2"], service, servicePath));
...
function insertDevices(cid, se, callback)
{
    ...
    var sqlse  = "INSERT INTO `temporary_devices`(`username`,`id`,`model`, `kind`,`devicetype`,`protocol`,`frequency`,`format`,`contextBroker`,`latitude`,`longitude`,macaddress,`status`,`validity_msg`,`should_be_registered`,`organization`,`edge_gateway_type`,`edge_gateway_uri`,`k1`,`k2`,`service`,`servicePath`) VALUES ?";
...
function storeDevice(user,deviceID,model,kind,type,protocol,frequency,format,cb,latitude,longitude,macaddress,status,validity_msg,shouldberegistered,organization,edge_type,edge_uri,k1,k2,service,servicePath){
    return [user, deviceID, model, kind, type, protocol, frequency, format, cb, latitude,longitude,macaddress,status,validity_msg,shouldberegistered,organization,edge_type,edge_uri,k1,k2,service,servicePath];
}
